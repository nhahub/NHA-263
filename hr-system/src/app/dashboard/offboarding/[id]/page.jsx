...
import { getOffboardingById, getAllEmployees } from "@/lib/api"
...
export default function OffboardingDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (!recordId || recordId === "undefined") {
      setError("Invalid offboarding record ID.")
      setLoading(false)
      return
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-purple-900 p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-cyan-400">Offboarding Record Details</h1>
            <p className="text-gray-300 text-sm">
              View offboarding information. Access rules are enforced on the server using your JWT.
            </p>
          </div>
        </div>
        ...
        <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700">
          <CardHeader>
            ...
            <CardDescription className="text-gray-400">
              All data for this offboarding record as returned by the API.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-base text-white font-medium">
                      {record.id || record.exitID || record.offboardingId || "-"}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-xs text-gray-400 uppercase mb-1">Resignation Date</p>
                    <p className="text-base text-white">
                      {formatDate(record.resignationDate)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 uppercase mb-1">Clearance Status</p>
                    <div className="flex items-center gap-2">
                      {getClearanceStatusBadge(record.clearanceStatus)}
                    </div>
                  </div>
                  <div className="md:col-span-2">
                    <p className="text-xs text-gray-400 uppercase mb-1">Exit Reason</p>
                    <p className="text-base text-white">
                      {record.exitReason || "-"}
                    </p>
                  </div>
                  {record.exitInterviewNotes && (
                    <div className="md:col-span-2">
                      <p className="text-xs text-gray-400 uppercase mb-1">Exit Interview Notes</p>
